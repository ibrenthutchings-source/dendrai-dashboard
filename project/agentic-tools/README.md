...
| `GITHUB_WRITE_TOKEN` | Closed-Loop Remediation | Personal access token with `repo` (push) scope. Lets `github_write_tool.py` open a real GitHub issue or pull request once a proposed remediation is manager-approved. Without it, `POST /remediation/propose(-pr)/{event_id}` still drafts and routes the proposal for approval, but the eventual write fails and surfaces as a retryable error in the Approval Inbox. |
...
The first write-capable connector primitive in this codebase — every `connector_poller.py` adapter is strictly `pull_events()`/`test_connection()`. Two proposal shapes, both routed through the same 2-stage preparer→manager approval workflow (`approvals_endpoints.py`) before anything is written:

- **Issue** (`gate_type='remediation_github'`) — the lowest-blast-radius action: opening a tracked ticket is fully reversible and touches no access grant. No file target needed.
- **Pull request** (`gate_type='remediation_github_pr'`) — a real file-change PR, for a finding the reviewer can point at a specific repo file (e.g. a DevOps Monitoring finding with a known misconfigured workflow/config file). There's no principled way to auto-detect which file an arbitrary business-exception finding maps to, so the reviewer names it explicitly.

**Flow (issue):** `POST /remediation/propose/{event_id}` (screen permission `approvals`, edit) drafts a GitHub issue title/body for one `exception_control_events` finding (LLM-drafted via `claude_client`, falls back to a plain templated issue if the call fails) and submits it for approval. A submission is always `disposition='adjusted'` — there's no "accept the finding as computed" path, so a remediation always requires a human review step before the write can fire.

**Flow (PR):** `POST /remediation/propose-pr/{event_id}` takes a `file_path` (+ optional `repo`/`base_branch`). `github_write_tool.get_file_content` reads the real file first — its failure (file doesn't exist, isn't valid UTF-8 text, etc.) is exactly what stops a bogus PR from ever being drafted. Claude then rewrites the complete file content to address the finding (`remediation_endpoints._draft_pr_fix`, capped at 40K input chars; unlike the issue path there is deliberately no safe templated fallback — a failed or no-op draft returns `None`, never a fabricated fix), and a unified diff (`difflib`) is computed and stored on the approval task for the manager to review before deciding.

Once a manager approves either kind (or, with no manager configured, immediately — same auto-approve reasoning `devops_scm_exception` already uses), `approvals_endpoints._execute_remediation` fires the real write — `github_write_tool.create_issue` or `create_pull_request` depending on `gate_type`. On success, the source finding is auto-resolved (`TRUE_CONTROL_FAILURE`, noting the issue/PR URL); on failure the error is persisted on the task (never swallowed) and the source finding stays open, surfaced in the Approval Inbox with a retry action.

**`github_write_tool.py`:** `create_issue(title, body, repo, labels)`; `get_file_content(path, repo, ref)` (Contents API, base64-decoded, rejects non-UTF-8 files); `create_pull_request(title, body, files, repo, base_branch)` — full Git Data API flow: blob → tree → commit → ref → PR. `test_connection()` verifies the token can both read and push before first use. Configured via `GITHUB_WRITE_TOKEN` / `GITHUB_REMEDIATION_REPO` (env vars, not the `poll_connectors` table — this is a write-only target with no events to poll).
...
| `POST` | `/remediation/propose-pr/{event_id}` | Draft a real file-change PR for one finding + a named file (`file_path`, optional `repo`/`base_branch`), submit for manager approval |
| `GET` | `/approvals/remediations` | Recent closed-loop remediation tasks (both gate types), any status |
| `POST` | `/approvals/remediations/{id}/retry` | Re-fire a failed remediation's GitHub write with the same approved content |

**Frontend:** `project/exceptions.jsx` — "Propose remediation" (issue) and "Propose PR fix" (opens an inline file-path input) buttons on a finding, showing status once proposed. `project/approval-inbox.jsx` renders `remediation_github`/`remediation_github_pr` tasks with distinct labels ("· GitHub Issue" / "· GitHub PR"), shows the proposed unified diff inline for a PR task awaiting review, and exposes the retry action for a failed write.
...
### Infrastructure Vulnerability & Currency Posture (`feat/infra-vuln-posture` branch — Development environment only)

Answers "has patching/CVE remediation been applied, and what is still open" — the question Infrastructure Monitoring above deliberately does not answer, since its checks are configuration posture (SSL on/off, public S3 buckets), not patch/CVE state. Development-environment-only (`deploy_env.IS_DEVELOPMENT`): a router-level 404 on every `/infra-posture/*` endpoint (`infra_posture_endpoints.py`, same pattern as Exception Management) and task-creation gates on both background sweeps, since a background loop has no HTTP request to 404.

**Core design:** a real stateful register (`observability.infra_assets` + `observability.infra_vulnerabilities`), not another `system_telemetry` event stream — "what's still open vs. remediated" needs a status column to flip, the same reasoning `risk_waivers`/`vendor_risk_profiles`/`itsm_tickets` each earned their own table for. `infra_assets.last_assessed_at IS NULL` is the load-bearing field: it's what keeps "never assessed" from rendering identically to "assessed and clean." Every summary count is coverage-qualified (assets assessed / total) for the same reason — see `db.get_vulnerability_summary`'s docstring.

- **Asset inventory + expiry** (`infra_asset_sweep.py`, daily): syncs `postgres_cis`/`tls_cert`/`aws_patch` connectors' current state into `infra_assets`, and raises a finding for any credential (`poll_connectors.credentials_expires_at`) or certificate inside its warning window (default 30d, `INFRA_EXPIRY_WARN_DAYS`).
- **Certificate expiry** (`tls_cert_tool.py`, connector_type `tls_cert`): a real TLS handshake against configured `host:port` endpoints — no credentials needed, the cheapest genuine networking coverage in this codebase.
- **Software currency, non-OSV** (`version_baselines.py`): OSV.dev has no PostgreSQL/generic-DB-engine ecosystem, so Postgres version currency (`iaas_connectors.fetch_postgres_config`'s `server_version`) is checked against a small hand-maintained baseline table instead — a currency check, never represented as a CVE check.
- **OSV.dev CVE enrichment** (`osv_client.py` + `vulnerability_sweep.py`, daily): for any asset with `ecosystem`+`software_name`+`software_version` set (populated by Phase 3 connectors below — nothing in Phase 1/2 sets `ecosystem`, since Postgres has none), batched querybatch + per-CVE detail hydration, cached in `observability.osv_cache` (24h TTL). Fails soft everywhere: an unreachable OSV leaves the asset's `last_assessed_at` untouched rather than looking like a clean scan.
- **Remediation detection**: a finding flips to `remediated` only when the sweep observes the asset's `software_version` has actually advanced past `fixed_version` (`remediation_basis='version_advanced'`) — never because a vuln simply stopped appearing in a scan. Absence of evidence is not evidence of remediation.
- **SARIF bridge** (`evidence_endpoints.py`): any SARIF finding ingested through the existing `/evidence/webhook` that carries a real CVE tag (container/dependency scanners — Trivy, Grype, Snyk — tag results this way) also projects into `infra_vulnerabilities` (`source='scanner'`), so scanner-sourced and OSV-sourced findings live in one register.
- **AWS SSM Patch Manager** (`aws_patch_tool.py`, connector_type `aws_patch`): real per-instance OS patch compliance (installed/missing/failed counts) via `ssm.describe_instance_patch_states` — the highest-value Phase 3 connector, since it's the one source that actually enumerates every managed instance regardless of findings (which is what lets it legitimately mark an instance "assessed").
- **AWS Inspector v2** (`aws_inspector_tool.py`, connector_type `aws_inspector`): real CVE findings with CVSS for EC2/ECR/Lambda via `inspector2.list_findings`, upserted directly into `infra_vulnerabilities` (`source='connector'`) with no OSV enrichment needed — AWS has already matched the package inventory. Deliberately never calls `mark_infra_asset_assessed`: `list_findings` only returns resources *with* findings, so a resource's absence from the response can't be read as "scanned clean."
- **Container image scanning**: no new backend code — any SARIF-producing scanner (Trivy, Grype) already lands through the existing `/evidence/webhook` + SARIF bridge above. This is a CI-pipeline wiring task (run the scanner, POST its SARIF output) rather than a connector, the same "any SARIF producer works, no per-tool code" property DevOps Monitoring's evidence ingestion already has.
- **Azure Update Manager/Defender, non-API network devices**: not built — Azure only pays for itself once customers are actually on Azure, and bare SSH/SNMP devices are out of scope for the same no-SSH-agent reason `iaas_connectors.py` documents for itself. Feasible only where a REST API exists (Meraki, Cisco DNA Center, Palo Alto PAN‑OS) — a future connector, not a gap in this feature's design.

**REST endpoints (prefix `/infra-posture`, Development environment only):** `GET /summary` (coverage-aware headline counts), `GET /assets`, `GET /vulnerabilities`, `POST /vulnerabilities/{id}/disposition` (`accepted_risk` creates a real `risk_waivers` row directly — not routed through the SCM exception flow's HITL gate, a deliberate scope simplification for a still-settling dev-only feature; `false_positive` requires a mandatory reason either way).

**Frontend:** `infrastructure-monitoring.jsx` gained Posture/Vulnerabilities/Asset Inventory/Expiry tabs, dev-only (`isDevEnv`), each surfacing the assets-assessed/total coverage note next to every open-finding count.

**Honest gaps:** cannot affirmatively prove "all patching is applied" — only "no known-open findings from connected sources," and every summary tile is phrased that way. Version→CVE matching (the OSV path) produces false positives on backported distro patches (RHEL/Debian bump security fixes without bumping the upstream version) — labeled "potential" until a distro-aware source or scanner confirms it. Coverage is bounded by what has an API; a bare on-prem VM or SSH-only device isn't reachable without an agent, which this platform deliberately avoids.
