...
def score_event(event_type: str, severity: str, raw_payload: Optional[dict],
    ...
    requires_human_review, model_version}. `event_type` is accepted for
    interface symmetry with the connector event shape and future per-type
    tuning, though only `severity` drives the score today."""
    rng = rng or random
