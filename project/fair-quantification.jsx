...
const _FAIR_RESOURCE_TYPES = [
  { id: "cem_event", label: "CEM Event", hint: "cem_events.id, e.g. 142" },
  { id: "cem_event_template", label: "CEM Event Template", hint: "cem_event_templates.id" },
  { id: "sox_process", label: "SOX Process", hint: "e.g. order_to_cash, procure_to_pay" },
  { id: "risk", label: "Risk Register Entry", hint: "risk_ref, e.g. R-strategic-01" },
  { id: "control", label: "Control", hint: "control_id, e.g. P2P-P002" },
];
...
function QuantifyForm({ onResult }) {
  ...
  const resourceMeta = _FAIR_RESOURCE_TYPES.find(r => r.id === resourceType);
  ...
  return (
    <form onSubmit={submit}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
        ...
        <div>
          <div style={labelStyle}>Resource reference — {resourceMeta.hint}</div>
          <input value={resourceRef} onChange={e => setResourceRef(e.target.value)} style={inputStyle} required />
        </div>
        <div>
          <div style={labelStyle}>Control ID — feeds empirical frequency (optional)</div>
          <input value={controlId} onChange={e => setControlId(e.target.value)} style={inputStyle} placeholder="e.g. P2P-P002" />
        </div>
        <div>
          <div style={labelStyle}>Lookback window (days)</div>
          <input type="number" min={1} value={windowDays} onChange={e => setWindowDays(e.target.value)} style={inputStyle} />
        </div>
      </div>

      <div className="kicker" style={{ marginBottom: 8 }}>Magnitude — first available wins</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 12 }}>
        <div>
          <div style={labelStyle}>SOX estimated exposure ($M)</div>
          <input type="number" step="0.01" value={soxExposure} onChange={e => setSoxExposure(e.target.value)} style={inputStyle} placeholder="from sox_process_details" />
        </div>
        <div>
          <div style={labelStyle}>Risk dollarExposureM ($M)</div>
          <input type="number" step="0.01" value={riskExposure} onChange={e => setRiskExposure(e.target.value)} style={inputStyle} placeholder="from Risk Flow" />
        </div>
        <div>
          <div style={labelStyle}>CEM severity default band</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 14 }}>
        <div>
          <div style={labelStyle}>Manual min ($M, optional override)</div>
  ...
  return (
    <div className="scope-screen" data-screen-label="Risk Quantification">
      ...
      <div style={{ border: "1px solid var(--line)", borderRadius: 8, padding: "16px 18px", marginBottom: 24, background: "var(--surface)" }}>
        ...
        <QuantifyForm onResult={handleNewResult} />
      </div>
      ...
      <div style={{ border: "1px solid var(--line)", borderRadius: 8, padding: "16px 18px" }}>
        ...
        <div className="panel-sub" style={{ marginBottom: 12 }}>
          Run two quantifications for the same resource — one reflecting the control absent or failing, one
          reflecting it holding — and compare their ALE here against the control's annual cost.
        </div>
